...
def record_and_trim(hotword_key, nb_records=3):
    ...
    directory = os.path.join(FOLDER_BASE, "personal_kws/")
...
global proc_start_wav

if __name__ == "__main__":
    try:
        signal.signal(signal.SIGINT, exit_gracefully)
        signal.signal(signal.SIGTERM, exit_gracefully)
        call(['sudo', 'systemctl', 'stop', 'snips-audio-server'])
        parser = argparse.ArgumentParser()
        parser.add_argument('hotword_key', type=str,
                            help="the name of your personal hotword (no special characters)")
        args = parser.parse_args()
        proc_start_wav = play_audio_file('start.wav')
        time.sleep(25)
        record_and_trim(args.hotword_key)
        os.system('sudo systemctl restart snips-hotword ; sudo systemctl restart snips-dialogue; sudo systemctl start snips-audio-server')
